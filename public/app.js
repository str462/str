...
function alternativeCards(r) {
  ...
  // No verified comparable product to name — rather than leaving this blank
  // or inventing a competing product, offer live comparison searches for the
  // exact item so the person can still explore alternatives themselves.
  const searches = fallbackAlternativeSearches(r);
  if (!searches.length) return '';
  return `<article class="alternative-card fallback-alt">
    <div class="alt-top"><span class="tag">Explore</span></div>
    <h3>No verified comparable product yet</h3>
    <p>We didn't find a reliably matching alternative to name directly. Compare it yourself with these live searches:</p>
    <div class="alt-links">${searches.map(s => `<a class="button secondary small" href="${safeUrl(s.url)}" target="_blank" rel="noopener noreferrer">${esc(s.label)} ↗</a>`).join('')}</div>
  </article>`;
}
  ...
  $('fullReport').innerHTML = `
    <div class="report-shell premium-report">
      ...
      <section class="report-section">
        <div class="row-between"><div class="section-label">VALUE BREAKDOWN</div>${r.scores_estimated ? '<span class="muted small-note">Some dimensions estimated from the overall score</span>' : ''}</div>
      ...
      <section class="report-section">
        ...
        <ul class="clean-list">${(r.positives||[]).length ? (r.positives||[]).map(x=>`<li>${esc(x)}</li>`).join('') : '<li>No specific advantage could be verified for this product yet.</li>'}</ul>
      </section>

      <section class="report-section">
        <div class="section-label">WHAT TO CONSIDER</div>
        <ul class="clean-list">${(r.concerns||[]).length ? (r.concerns||[]).map(x=>`<li>${esc(x)}</li>`).join('') : '<li>No specific concern could be verified for this product yet — that\'s not the same as a clean bill of health, just limited evidence.</li>'}</ul>
      </section>

      <section class="report-section">
        <div class="section-label">IMPORTANT BEFORE BUYING</div>
        <ul class="clean-list">${(r.important_to_know||[]).length ? (r.important_to_know||[]).map(x=>`<li>${esc(x)}</li>`).join('') : '<li>Double-check seller reputation, return policy and warranty before paying — no site-specific detail was verified for this listing.</li>'}</ul>
