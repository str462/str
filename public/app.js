...
function productImage(url, title='Product', cls='product-image') {
  if (!url) return `<div class="${cls} image-placeholder" aria-label="${esc(title)}"><span>✦</span></div>`;
  return `<div class="${cls}"><img src="${safeUrl(url)}" alt="${esc(title)}" loading="lazy" referrerpolicy="no-referrer" onerror="this.closest('.${cls}')?.classList.add('image-failed');this.remove()"></div>`;
}
    ...
    <div class="report-shell free-report">
      ...
      <div class="premium-teaser">
        ...
        <div class="locked-report-grid">
          <article class="locked-card visual-lock">${productImage(null,'Best offer','locked-image')}<span>🔒 PREMIUM</span><h3>Best place to buy</h3><p>Current offer, seller, price and buying link.</p></article>
          <article class="locked-card visual-lock"><div class="fake-alt-stack"><span></span><span></span><span></span></div><span>🔒 PREMIUM</span><h3>Better alternatives</h3><p>Comparable products with images, prices and reasons.</p></article>
          <article class="locked-card visual-lock"><div class="risk-orb">!</div><span>🔒 PREMIUM</span><h3>Buying risks</h3><p>Seller, warranty, returns and important details.</p></article>
          <article class="locked-card visual-lock"><div class="target-preview">${esc(target)}</div><span>🔒 PREMIUM</span><h3>Your target price</h3><p>A practical price to wait for before you buy.</p></article>
        </div>
        <div class="soft-paywall">
          <div class="paywall-icon">✦</div>
          <div><div class="eyebrow">ONE SIMPLE UPGRADE</div><h3>Know the better option before you pay.</h3><p>Unlock the complete buying decision for <b>$0.99 today</b>.</p><div class="offer-inline"><b><s>$2.99</s> $0.99 today</b><span>3-day Premium starter · then $35/month</span></div></div>
  ...
  $('fullReport').innerHTML = `
    <div class="report-shell premium-report">
      ...
      <div class="recommendation ${recClass}">
        <div class="rec-main"><span>OUR RECOMMENDATION</span><strong>${esc(rec)}</strong><p>${esc(r.summary || '')}</p></div>
      ...
      <section class="report-section">
        ...
        <div class="score-grid">${Object.entries(r.scores || {}).map(([k,v]) => `<div class="mini-score"><span>${esc(k.replace(/_/g,' '))}</span><b>${v == null ? '—' : Math.round(v)}</b><div class="bar"><i style="width:${v == null ? 0 : Math.max(0,Math.min(100,v))}%"></i></div></div>`).join('')}</div>
      </section>

      <section class="report-section">
        <div class="section-label">04 · BETTER OPTIONS</div>
      ...
      <section class="report-section">
        <div class="row-between"><div class="section-label">05 · PRICE EVIDENCE</div><span class="muted">${(r.evidence||[]).length} source(s)</span></div>
...
window.checkSavedPrice = async (index) => {
  ...
  try {
    ...
    if(Number(item.currentPrice)<=Number(item.targetPrice) && 'Notification' in window && Notification.permission==='granted'){
      new Notification('DEALCHECK: target reached',{body:`${item.title} is now ${money(item.currentPrice,item.currency)} — at or below your target.`});
    }
