...
function getPremiumPreviewToken() {
  try {
    const stored = sessionStorage.getItem('dealcheck_premium_preview_token') || '';
    if (stored) return stored;
    const hash = String(location.hash || '');
    const match = hash.match(/(?:^#|&)premium-preview=([^&]+)/i);
    if (match && match[1]) {
      const token = decodeURIComponent(match[1]);
      if (token) sessionStorage.setItem('dealcheck_premium_preview_token', token);
      return token;
    }
  } catch (_) {}
  return '';
}
function isPremiumPreview() { return Boolean(getPremiumPreviewToken()); }
function isPremium() {
  return isPremiumPreview() || localStorage.getItem('dealcheck_premium') === 'true';
}

function setPremiumState(data) {
  const premium = Boolean(data?.premium) || isPremiumPreview();
...
async function refreshPremiumStatus() {
  const previewToken = getPremiumPreviewToken();
  ...
  try {
    const resp = await fetch('/api/analyze', {
      ...
      body: JSON.stringify({
        ...
        previewToken: getPremiumPreviewToken()
      })
    });
