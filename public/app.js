...
function getAttribution() {
  ...
  const current = {};
  keys.forEach(k => { const v = params.get(k); if (v) current[k] = v; });
  const saved = JSON.parse(localStorage.getItem('dealcheck_attribution') || '{}');
  const merged = { ...saved, ...current };
  if (Object.keys(current).length) localStorage.setItem('dealcheck_attribution', JSON.stringify(merged));
  return merged;
}
function track(name, data={}) {
  const events = JSON.parse(localStorage.getItem('dealcheck_events') || '[]');
  const event = { name, at: new Date().toISOString(), path: location.pathname, ...getAttribution(), ...data };
  events.push(event);
  localStorage.setItem('dealcheck_events', JSON.stringify(events.slice(-500)));
  // Optional server-side event logging. It never blocks the user flow.
  try {
    const body = JSON.stringify({ name, at:event.at, path:event.path, attribution:getAttribution(), data });
    if (navigator.sendBeacon) navigator.sendBeacon('/api/track', new Blob([body], {type:'application/json'}));
    else fetch('/api/track',{method:'POST',headers:{'Content-Type':'application/json'},body,keepalive:true}).catch(()=>{});
  } catch (_) {}
  // Optional Meta Pixel integration. Set NEXT_PUBLIC_META_PIXEL_ID in Vercel.
  if (window.fbq) {
    const standard = {
      landing_view:'PageView',
      analysis_started:'ViewContent',
      paywall_open:'InitiateCheckout',
      payment_success:'Purchase'
    }[name];
    try { window.fbq(standard || 'trackCustom', standard || name, data); } catch (_) {}
  }
}
...
async function initOptionalMetaPixel() {
  try {
    const r = await fetch('/api/config', { cache:'no-store' });
    if (!r.ok) return;
    const cfg = await r.json();
    ...
    if (localStorage.getItem('dealcheck_marketing_consent') === 'granted') {
      load();
      return;
    }
    if (localStorage.getItem('dealcheck_marketing_consent') === 'denied') return;
    if ($('analyticsConsent')) return;
    const banner = document.createElement('div');
    banner.id = 'analyticsConsent';
    banner.className = 'analytics-consent';
    banner.innerHTML = `<div><b>Optional analytics</b><p>${uiText('We use optional analytics to understand which parts of the site help visitors.')}</p></div><div class="analytics-consent-actions"><button id="analyticsDecline" type="button" class="button secondary small">${uiText('Not now')}</button><button id="analyticsAllow" type="button" class="button primary small">${uiText('Allow analytics')}</button></div>`;
    document.body.appendChild(banner);
    $('analyticsAllow').onclick = () => {
      localStorage.setItem('dealcheck_marketing_consent','granted');
      banner.remove();
      load();
    };
    $('analyticsDecline').onclick = () => {
      localStorage.setItem('dealcheck_marketing_consent','denied');
      banner.remove();
    };
  } catch (_) {}
}
initOptionalMetaPixel();
...
$('advancedToggle')?.addEventListener('click', () => {
  ...
  $('advancedToggle').querySelector('b').textContent = open ? '＋' : '−';
  if (!open) track('advanced_options_opened');
});
$('urlInput')?.addEventListener('input', () => {
  $('clearUrlBtn')?.classList.toggle('hidden', !$('urlInput').value.trim());
  if ($('urlInput').value.trim() && sessionStorage.getItem('dealcheck_form_started') !== '1') {
    sessionStorage.setItem('dealcheck_form_started','1');
    track('check_form_started');
  }
});
...
const EXAMPLE_URL = 'https://www.amazon.com/Sony-WH-1000XM5-Canceling-Headphones-Hands-Free/dp/B09XS7JWHH2';
$('exampleLinkBtn')?.addEventListener('click', () => {
  $('urlInput').value = EXAMPLE_URL;
  $('clearUrlBtn')?.classList.remove('hidden');
  track('example_link_used');
  $('urlInput').focus();
});
function closeDemoModal() { $('demoModal')?.classList.add('hidden'); }
...
$('demoCheckBtn')?.addEventListener('click', () => {
  closeDemoModal();
  navigate('home');
  $('urlInput').focus();
  $('urlInput').scrollIntoView({behavior:'smooth',block:'center'});
  track('demo_to_real_check');
});
...
$('analyzeBtn').addEventListener('click', async () => {
  const url = normalizeProductUrl($('urlInput').value);
  ...
  track('analysis_started', { input_type: activeTab === 'shot' ? 'screenshot' : (url ? 'url' : 'manual'), country, language: window.PriceCheckrI18n ? window.PriceCheckrI18n.getLocale() : 'en' });
  try {
    ...
    if (verifiedPremium) {
      renderFull(report);
      track('analysis_completed', { premium: true, product: productTitle(report), country });
      track('premium_result');
    } else {
      renderFree(report);
      localStorage.setItem('dealcheck_free_used', 'true');
      $('resultSection').classList.remove('hidden');
      track('analysis_completed', { premium: false, product: productTitle(report), country });
      track('free_result');
      $('resultSection').scrollIntoView({ behavior:'smooth', block:'start' });
    }
  } catch (e) {
    track('analysis_failed', { message: String(e?.message || 'Analysis failed.').slice(0, 180), country });
...
async function mountPaymentForm() {
  ...
  try {
    ...
    track('checkout_started', { offer:'0.99_today_3_day_trial_35_month' });
...
async function confirmStarterPayment() {
  ...
  try {
    ...
    track('payment_failed', { message: String(err?.message || 'Payment could not be completed.').slice(0, 180) });
...
window.checkSavedPrice = async (index) => {
  ...
  if (!item) return;
  const url = normalizeProductUrl(item.sourceUrl || item.report?._sourceUrl || item.report?.best_place_to_buy?.url || '');
