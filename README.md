DEALCHECK 3.0 — premium shopping decision experience
...
- `OPENAI_MODEL` — an API model ID enabled in your OpenAI project. If blank or unset, the code falls back to a supported Responses API model. Use an API model ID that exists in your OpenAI project.
...
## No-payment Premium preview

## Conversion tracking (optional)

The frontend now tracks the funnel locally and sends lightweight events to `/api/track` for Vercel server logs:
`landing_view`, `check_form_started`, `analysis_started`, `analysis_completed`, `analysis_failed`, `demo_opened`, `demo_to_real_check`, `paywall_open`, `checkout_started`, `payment_success`, and `payment_failed`.
UTM parameters and common `fbclid`/`gclid` attribution are retained for the session/device.

For Meta Ads conversion tracking, optionally add this Vercel environment variable:
`NEXT_PUBLIC_META_PIXEL_ID=<your Meta Pixel ID>`

The pixel is not activated until the visitor chooses **Allow analytics**. No secret API key is placed in the browser.
