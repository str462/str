...
For testing only, set `PREMIUM_PREVIEW_TOKEN` in Vercel to a private random value. Then open the site with `#premium-preview=YOUR_TOKEN`. The token is read from the URL fragment (not sent in the initial page request), and Premium analysis/price checks are accepted server-side only when the token matches the Vercel environment variable. Remove or rotate this variable before production launch.
