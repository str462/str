...
module.exports = async function handler(req, res) {
  ...
  try {
    const previewToken = String(req.query.previewToken || '').trim();
    const configuredPreviewToken = String(process.env.PREMIUM_PREVIEW_TOKEN || '').trim();
    if (configuredPreviewToken && previewToken && previewToken === configuredPreviewToken) {
      return res.status(200).json({ premium: true, status: 'preview', subscriptionId: null, customerId: null, email: null, name: 'Premium Preview', trialEnd: null, currentPeriodEnd: null, cancelAtPeriodEnd: false, preview: true });
    }
