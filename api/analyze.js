...
async function hasActivePremium(subscriptionId, customerId, previewToken) {
  const configuredPreviewToken = String(process.env.PREMIUM_PREVIEW_TOKEN || '').trim();
  if (configuredPreviewToken && previewToken && previewToken === configuredPreviewToken) return true;
...
`;
    ...
    const previewToken = typeof body.previewToken === 'string' ? body.previewToken.trim().slice(0, 300) : '';
    const premium = await hasActivePremium(subscriptionId, customerId, previewToken);
