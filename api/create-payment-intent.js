...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Client-generated key, stable per checkout attempt (see getOrCreateIntentAttemptKey
    // in app.js). Used as a Stripe idempotency key so that if this request is ever sent
    // twice — a race on the client, a retried fetch, a flaky connection — Stripe returns
    // the SAME PaymentIntent instead of creating and potentially charging a second one.
    const intentAttemptKey = String(body.intentAttemptKey || '').trim().slice(0, 200);
    ...
    const customer = await stripe.customers.create(
      ...
      intentAttemptKey ? { idempotencyKey: `dealcheck-customer-${intentAttemptKey}` } : undefined
    );
    ...
    const paymentIntent = await stripe.paymentIntents.create(
      ...
      intentAttemptKey ? { idempotencyKey: `dealcheck-pi-${intentAttemptKey}` } : undefined
    );
